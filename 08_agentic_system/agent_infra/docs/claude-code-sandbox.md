...
- [Claude Code Sandbox 安全隔离机制解析](#claude-code-sandbox-安全隔离机制解析)
  ...
  - [3 结合 Claude Code 源码：Bubblewrap 的实际应用示例](#3-结合-claude-code-源码bubblewrap-的实际应用示例)
    ...
    - [3.3 深入探讨：沙盒的生命周期与执行细节](#33-深入探讨沙盒的生命周期与执行细节)
      - [3.3.1 沙盒生命周期：极高频的“即用即毁”模式](#331-沙盒生命周期极高频的即用即毁模式)
      - [3.3.2 Sandbox 中到底执行了哪些操作？](#332-sandbox-中到底执行了哪些操作)
      - [3.3.3 底层架构权衡：轻量级沙盒与标准容器的取舍](#333-底层架构权衡轻量级沙盒与标准容器的取舍)
...
代码参考位置：`src/utils/sandbox/sandbox-adapter.ts` (Claude Code 源码，L19)
...
代码参考位置：`src/utils/Shell.ts` (Claude Code 源码，L248)
