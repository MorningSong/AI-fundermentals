...
[check_links.py](./tools/check_links.py) 是一个基于 Python 开发的自动化工具，专门用于验证项目内 Markdown 文档中本地及外部超链接的有效性。该工具不仅支持处理包含空格、URL 编码、Git 子模块未初始化等复杂场景，还全面兼容 HTML 图片标签（`<img>`）的解析，并内置了针对外部 URL 的反爬机制重试逻辑。此外，它支持通过参数灵活选择检查范围（全量扫描、递归目录检查或针对特定文件），并支持简写参数（如 `-d`、`-f`、`-a`、`-t`）。详细的参数说明与使用示例，请参阅 [工具使用指南](./tools/README.md)。
