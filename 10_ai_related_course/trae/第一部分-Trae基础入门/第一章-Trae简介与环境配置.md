...
在享受 AI 编程效率的同时，你需要掌握一项关键技能：**审查 AI 生成的代码**。AI 不是万能的——它会犯错、会使用过时的 API、会引入安全漏洞、也会产生逻辑错误。下面是在每个 AI 生成环节之后你应该检查的内容。

### 8.1 四步审查法

拿到 AI 生成的代码后，按以下顺序检查：
...
### 8.2 实战：审查番茄钟应用的 AI 生成代码

回顾我们之前构建的番茄钟应用，AI 生成的代码中至少有以下几个点值得你检查：
...
**关键教训**：AI 生成的 `setInterval` 往往不会帮你做清理。你需要识别这类问题并修正。

### 8.3 AI 更容易出错的场景

- **版本特定的 API**：AI 的知识截止日期有限，可能使用已被废弃的 API
- **状态管理**：复杂的状态流转，AI 容易遗漏边界条件
- **异步逻辑**：Promise/async-await 的竞态条件和错误处理
- **安全性**：用户输入校验、认证授权逻辑
- **性能**：大列表渲染、不必要的重计算

> **记住**：AI 是你的加速器，但**你是驾驶员**。每一行 AI 生成的代码，最终都需要你来负责。

---

## 9. 小结

本章介绍了 Trae 作为 AI-Native IDE 的核心优势，特别是 **SOLO 模式** 和 **Agent 机制**。通过亲手构建番茄钟应用，大家应该已经感受到了”自然语言编程”的高效。同时，请牢记第一节的审查方法论——高效的前提是可控。在下一章，我们将深入探索更复杂的项目开发技巧。
