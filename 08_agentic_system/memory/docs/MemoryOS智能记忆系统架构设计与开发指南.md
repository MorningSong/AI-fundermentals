...
> [!NOTE]
> `get_response()` 的 `style_hint` 参数目前已预留但尚未实现。如需注入自定义 System Prompt，当前版本需直接修改 `prompts.py` 中的模板。
...
编排模块是连接三层记忆的纽带。Updater 封装了写入路径的逻辑，包括对话存储、连续性检查和摘要生成；Retriever 封装了读取路径，使用并行检索提升响应速度。中期到长期的升级（画像分析和知识提取）由 `Memoryos` 主类在检测到热度阈值后触发。这种设计使得记忆模块保持纯粹的存储职责，而复杂的业务逻辑集中在编排层。

| 模块   | 类          | 职责                    | 依赖                    |
| ------ | ----------- | ----------------------- | ----------------------- |
| 更新器 | `Updater`   | 记忆写入、短期→中期升级 | 短期/中期/长期记忆, LLM |
| 检索器 | `Retriever` | 并行检索、结果聚合      | 中期/长期记忆           |

- **Updater**：在短期记忆满容量时执行 `process_short_term_to_mid_term()`。升级过程详见第 4.3 节。
- **Retriever**：`retrieve_context()` 使用 `ThreadPoolExecutor(max_workers=3)` 并行执行：
  1. `_retrieve_mid_term_context()` - 从中期记忆检索页面（堆排序 Top-K）
  2. `_retrieve_user_knowledge()` - 从用户 LTM 检索知识
  3. `_retrieve_assistant_knowledge()` - 从助手 LTM 检索知识
...
3. 无论是否触发升级，均调用 `_trigger_profile_and_knowledge_update_if_needed()` 检查并触发热度驱动的 LTM 更新
...
读取流程核心设计是"并行检索 + 上下文融合"。系统使用 `ThreadPoolExecutor` 同时检索中期页面、用户知识和助手知识，将结果与短期对话历史融合后调用 LLM 生成响应。响应生成后会自动写入短期记忆，形成闭环。

流程由 `Memoryos.get_response()` 方法编排：

1. 调用 `Retriever.retrieve_context()` 并行检索中期页面、用户知识、助手知识
2. 调用 `ShortTermMemory.get_all()` 获取对话历史
3. 调用 `LongTermMemory.get_raw_user_profile()` 获取用户画像
4. 组装 Prompt（System Prompt + User Prompt）
5. 调用 LLM 生成响应
6. 调用 `add_memory()` 将本次对话写入短期记忆
...
热度超过阈值后，`Memoryos._trigger_profile_and_knowledge_update_if_needed()` 使用 `ThreadPoolExecutor(max_workers=2)` 并行执行：

1. `gpt_user_profile_analysis()` - LLM 分析用户画像 → 更新 `user_long_term_memory.user_profiles`
2. `gpt_knowledge_extraction()` - LLM 提取知识 → 分别写入 `user_long_term_memory.knowledge_base` 和 `assistant_long_term_memory.assistant_knowledge`
3. 重置会话 `N_visit` 和 `L_interaction`，标记所有页面为已分析（`analyzed=True`），重新计算 `H_segment`
