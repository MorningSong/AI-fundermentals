...
tools/memory_tool.py     ← MemoryStore + memory 工具
       ↑
agent/builtin_memory_provider.py  ← 内建适配器
       ↑
agent/memory_manager.py  ← 编排 builtin + 外部 provider
       ↑
...
- `MEMORY.md`：2,200 字符（约 800 tokens，按 2.75 chars/token 估算）
...
| `get_config_schema()`                      | 设置向导中      | 返回配置字段定义             |
...
`agent/memory_manager.py` 是内建 provider 与外部 provider 之间的编排层。它的核心约束是：**至多一个外部 provider**。
...
- **哨兵标记清理**：使用唯一哨兵 `_sentinel` 而非身份检查来定位和清理冲刷相关的消息，更加健壮。
...
# Step 3: 创建 MemoryManager 并注册 provider
...
# Step 4: 初始化所有 provider
self._memory_manager.initialize_all(session_id=self.session_id, platform=self.platform)
```
...
- 内建记忆通过 `format_for_system_prompt()` 获取冻结快照，在整个 session 中保持稳定
- 外部 provider 的 `system_prompt_block()` 可以返回动态内容（如 Honcho 的 peer card 摘要）
...
| **RetainDB**    | 云端 DB   | Delta 压缩 + 持久知识库                       | -        | 存储效率         |
| **Supermemory** | 云端 API  | 上下文围栏 + 会话图                           | -        | 多 session 关联  |
| **ByteRover**   | 混合      | 预压缩提取                                    | -        | 成本优化         |
| **OpenViking**  | 自托管 FS | 文件系统层级存储                              | -        | 完全本地控制     |
...
Holographic 是唯一的零外部依赖 provider，使用 SQLite 存储事实，并通过 HRR（Holographic Reduced Representations）代数实现组合式检索：
